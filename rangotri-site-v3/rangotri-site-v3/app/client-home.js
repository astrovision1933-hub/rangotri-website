...
function ChatWidget({ open, onClose, config }) {
  ...
  if (!open) return null;
  const { chatbox, background, accent, card, text } = config.colors;

  return (
    <div style={{ position: "fixed", bottom: 88, right: 20, width: 340, maxWidth: "calc(100vw - 32px)", background: card, border: `2px dashed ${chatbox}`, borderRadius: 6, boxShadow: "0 10px 30px rgba(27,51,80,0.25)", overflow: "hidden", zIndex: 1000, fontFamily: "'Inter', sans-serif" }}>
      <div style={{ background: chatbox, color: background, padding: "10px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontFamily: "'Oswald', sans-serif", fontWeight: 600, fontSize: 14, letterSpacing: 0.5 }}>ORDER DESK</span>
        <button onClick={onClose} style={{ background: "none", border: "none", color: background, cursor: "pointer", padding: 2 }}><X size={16} /></button>
      </div>

      <div ref={scrollRef} style={{ height: 300, overflowY: "auto", padding: "12px", display: "flex", flexDirection: "column", gap: 8 }}>
        {messages.map((m, i) => (
          <div key={i} style={{ alignSelf: m.role === "user" ? "flex-end" : "flex-start", maxWidth: "85%", background: m.role === "user" ? chatbox : "#EFE0C8", color: m.role === "user" ? background : text, padding: "8px 11px", borderRadius: m.role === "user" ? "10px 10px 2px 10px" : "10px 10px 10px 2px", fontSize: 13.5, lineHeight: 1.4, whiteSpace: "pre-wrap" }}>
            {m.content}
          </div>
        ))}
      ...
      <div style={{ padding: 10, borderTop: "1px solid #1B335022", background: "#F5EEDF" }}>
        <div style={{ fontSize: 10, fontWeight: 700, color: chatbox, textTransform: "uppercase", marginBottom: 6 }}>Order confirm? Owner ko WhatsApp alert bhejein</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
          ...
          {alertSent && <div style={{ fontSize: 11, color: chatbox, textAlign: "center" }}>✓ WhatsApp khul gaya, bas send dabao</div>}
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 6, padding: "8px 10px", borderTop: "1px solid #1B335022" }}>
        ...
        <button onClick={() => send()} disabled={loading} style={{ background: accent, color: "#fff", border: "none", borderRadius: "50%", width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}>
          <Send size={14} />
        </button>
      </div>
    </div>
...
export default function BusinessWebsite({ config }) {
  ...
  const { header, footer, background, accent, text, card } = config.colors;
  const PROCESS = [
  ...
  return (
    <div style={{ background: background, color: text, fontFamily: "'Inter', sans-serif", minHeight: "100vh" }}>
      <style>{`
        .display { font-family: 'Oswald', sans-serif; }
        .mono { font-family: 'JetBrains Mono', monospace; }
        .swing-tag { background: ${card}; border: 2px dashed ${header}; border-radius: 4px; position: relative; }
        .swing-tag::before { content: ''; position: absolute; top: -8px; left: 20px; width: 14px; height: 14px; border-radius: 50%; background: ${background}; border: 2px solid ${header}; }
        .nav-link { display: inline; }
        @media (max-width: 700px) { .nav-link { display: none; } }
        a { color: inherit; }
      `}</style>

      <header style={{ background: header, color: background, padding: "16px 6vw", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: `3px solid ${accent}`, position: "sticky", top: 0, zIndex: 100 }}>
        <div className="display" style={{ fontWeight: 700, fontSize: 20, letterSpacing: 1 }}>
          {config.businessName.split(" ")[0].toUpperCase()} <span style={{ color: accent }}>{config.businessName.split(" ").slice(1).join(" ").toUpperCase()}</span>
        </div>
        <nav style={{ display: "flex", gap: 28, alignItems: "center", fontSize: 13.5 }}>
          <a href="#fabrics" className="nav-link">Fabrics</a>
          <a href="#process" className="nav-link">Process</a>
          <a href="#contact" className="nav-link">Contact</a>
          <button onClick={() => setChatOpen(true)} className="display" style={{ background: accent, color: "#fff", border: "none", padding: "8px 16px", borderRadius: 4, fontWeight: 600, letterSpacing: 0.5, cursor: "pointer", fontSize: 13 }}>
            GET A QUOTE
          </button>
        </nav>
      </header>
      ...
      <section style={{ padding: "72px 6vw 56px", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", inset: 0, opacity: 0.05, backgroundImage: `repeating-linear-gradient(45deg, ${header} 0, ${header} 2px, transparent 2px, transparent 14px)` }} />
        <div style={{ maxWidth: 720, position: "relative" }}>
          <div className="mono" style={{ fontSize: 12, letterSpacing: 2, color: accent, marginBottom: 14, textTransform: "uppercase" }}>
          ...
          <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
            <button onClick={() => setChatOpen(true)} className="display" style={{ background: header, color: background, border: "none", padding: "13px 26px", borderRadius: 4, fontWeight: 600, letterSpacing: 0.5, cursor: "pointer", fontSize: 14 }}>
              REQUEST A SAMPLE
            </button>
            <a href="#fabrics" className="display" style={{ border: `2px solid ${header}`, padding: "11px 24px", borderRadius: 4, fontWeight: 600, letterSpacing: 0.5, fontSize: 14, textDecoration: "none" }}>
              VIEW FABRICS
            </a>
          </div>
        </div>
      ...
      <section style={{ background: header, color: background, padding: "28px 6vw", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 20 }}>
        {config.stats.map((s) => (
          <div key={s.label}>
            <div className="mono" style={{ fontSize: 26, fontWeight: 700, color: accent }}>{s.value}</div>
      ...
      <section id="fabrics" style={{ padding: "64px 6vw" }}>
        <div className="mono" style={{ fontSize: 12, letterSpacing: 2, color: accent, marginBottom: 8, textTransform: "uppercase" }}>Fabric Book</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(230px, 1fr))", gap: 24 }}>
          {config.fabrics.map((f) => (
            <div key={f.tag} className="swing-tag" style={{ padding: "24px 20px" }}>
              ...
              <div className="mono" style={{ fontSize: 13, color: accent, marginBottom: 10 }}>{f.gsm}</div>
      ...
      <section id="process" style={{ padding: "64px 6vw", background: card }}>
        <div className="mono" style={{ fontSize: 12, letterSpacing: 2, color: accent, marginBottom: 8, textTransform: "uppercase" }}>How An Order Runs</div>
            ...
            return (
              <div key={n}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                  <span className="mono" style={{ fontSize: 13, color: accent, fontWeight: 700 }}>{n}</span>
                  <Icon size={18} color={header} />
                </div>
      ...
      <section style={{ padding: "64px 6vw", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 32 }}>
        <div>
          <Award size={22} color={accent} style={{ marginBottom: 10 }} />
        ...
        <div>
          <Factory size={22} color={accent} style={{ marginBottom: 10 }} />
        ...
        <div>
          <Truck size={22} color={accent} style={{ marginBottom: 10 }} />
      ...
      <section id="contact" style={{ background: footer, color: background, padding: "56px 6vw 40px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 32, marginBottom: 32 }}>
          <div>
            ...
            <button onClick={() => setChatOpen(true)} className="display" style={{ background: accent, color: "#fff", border: "none", padding: "11px 22px", borderRadius: 4, fontWeight: 600, cursor: "pointer", fontSize: 13.5 }}>
      ...
      {!chatOpen && (
        <button onClick={() => setChatOpen(true)} style={{ position: "fixed", bottom: 20, right: 20, background: accent, color: "#fff", border: "none", borderRadius: "50%", width: 54, height: 54, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", boxShadow: "0 6px 18px rgba(0,0,0,0.25)", zIndex: 999 }}>
